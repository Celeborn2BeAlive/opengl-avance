...
void main()
{
	...
	vec3 gnormal = vec3(texelFetch(uGNormal, ivec2(gl_FragCoord.xy), 0));
	vec3 ka = vec3(texelFetch(uGAmbient, ivec2(gl_FragCoord.xy), 0));
	vec3 kd = vec3(texelFetch(uGDiffuse, ivec2(gl_FragCoord.xy), 0));
	float shininess = vec3(texelFetch(uGlossyShininess, ivec2(gl_FragCoord.xy), 0)).x;

    vec3 normal = normalize(gnormal);
    vec3 eyeDir = normalize(-gposition);
    ...
    fColor += (uDirectionalLightIntensity * dothDirLight + pointLightIncidentLight * dothPointLight);
}